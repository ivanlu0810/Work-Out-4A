...
-- 檢查現有資料表的完成狀態欄位
SHOW COLUMNS FROM workout_exercises LIKE '%completion%';
SHOW COLUMNS FROM workout_sessions LIKE '%completion%';
SHOW COLUMNS FROM training_plans LIKE '%completion%';
...
-- - 索引優化查詢效能
-- - 使用簡單的 SHOW 查詢避免保留字問題

-- 後續步驟：
-- 1. 執行 SHOW TABLES 和 SHOW COLUMNS 查詢確認建立成功
-- 2. 測試插入和查詢功能
-- 3. 如需要，可以手動設定外鍵約束
-- 4. 整合到前端介面中
